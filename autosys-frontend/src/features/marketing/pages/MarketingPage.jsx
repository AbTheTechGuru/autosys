...
const STATUS_COLOR = { Active: G.ok, Completed: G.bl, Scheduled: G.wa, Draft: G.t2, active: G.ok, completed: G.bl, scheduled: G.wa, draft: G.t2 };

const BUILT_IN_TEMPLATES = [
  { name:'Flash Sale Blast',   type:'whatsapp',  desc:'Urgent discount offer with emoji and CTA',    color:'#25D366' },
  { name:'New Arrival Alert',  type:'whatsapp',  desc:'Announce new vehicle in stock',               color:'#25D366' },
  { name:'Monthly Newsletter', type:'email',     desc:'Professional HTML email digest',              color:G.bl      },
  { name:'Instagram Caption',  type:'instagram', desc:'Engaging visual post with hashtags',          color:'#E1306C' },
  { name:'Follow-Up SMS',      type:'sms',       desc:'Short follow-up for uncontacted leads',       color:G.wa      },
];
...
function CampaignWizard({ open, onClose, initialForm = {} }) {
  ...
  const [form, setForm] = useState({
    name: '', type: 'whatsapp', audience: 'all', msg: '', schedule: 'Now', ...initialForm,
  });
  ...
  // Reset when closed
  useEffect(() => {
    if (!open) { setStep(0); setErrors({}); setAiLoad(false); }
  }, [open]);
  ...
  const generateMsg = async () => {
    ...
    try {
      ...
      setForm((f) => ({ ...f, msg: '🚗 Huge deals this January! Premium vehicles at unbeatable prices. Visit us — limited slots available!' }));
    }
    setAiLoad(false);
  };
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      toast(err.response?.data?.message || 'Launch failed', 'danger');
    } finally {
    ...
    <Modal open={open} onClose={onClose} title="New Campaign" maxWidth={560}>
      ...
      <div className="flex gap-1 mb-5">
        {WIZARD_STEPS.map((label, i) => (
          <div key={label} className="flex-1 flex flex-col items-center gap-1">
            <div className="w-[28px] h-[28px] rounded-full flex items-center justify-center text-[12px] font-extrabold"
        ...
        <div className="flex flex-col gap-3">
          ...
          <Field label="Channel *">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[['whatsapp','#25D366','wa','WhatsApp'],['email',G.bl,'mail','Email'],['instagram','#E1306C','img','Instagram'],['sms',G.wa,'send','SMS']].map(([type, color, icon, label]) => (
                <button key={type} onClick={() => set('type')(type)}
                  className={cn('flex flex-col items-center gap-[5px] px-2 py-[11px] rounded-[9px] border cursor-pointer transition-all font-sans',
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-surface-3 rounded-[10px] p-4 border border-surface-4">
            ...
            <div className="font-display text-[28px] font-bold" style={{ color: G.g }}>
              {form.audience === 'all' ? '—' : form.audience === 'new_leads' ? '—' : '—'}
            </div>
            <div className="text-[12px] text-text-muted">contacts (real count loads from CRM)</div>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="flex flex-col gap-3">
          ...
          <Textarea rows={5} placeholder="Type or generate with AI…" value={form.msg} onChange={set('msg')} />
      ...
      {step === 3 && (
        <div>
          ...
          {[['Name',form.name||'Untitled'],['Channel',form.type],['Audience',form.audience],['Preview',form.msg.slice(0,80)+(form.msg.length>80?'…':'')]].map(([k,v]) => (
      ...
      <div className="flex justify-between mt-5">
        ...
        <Button variant="gold" disabled={isLaunching}
          onClick={() => step === WIZARD_STEPS.length - 1 ? handleLaunch() : setStep((s) => s + 1)}>
          {step === WIZARD_STEPS.length - 1
            ? (isLaunching ? <><Spinner size={13} />Launching…</> : <><Icon name="send" size={14} />Launch Campaign</>)
            : 'Continue →'}
        </Button>
      </div>
    </Modal>
      ...
      {tab === 'campaigns' && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            {[['Sent',stats.sent.toLocaleString(),G.bl],['Opens',stats.opens.toLocaleString(),G.g],['Click Rate','—',G.ok],['Campaigns',campaigns.length,G.er]].map(([l,v,c]) => (
          ...
          {isLoading ? (
            <div className="flex flex-col gap-3">
              {Array(3).fill(0).map((_, i) => (
                <div key={i} className="h-[130px] bg-surface-2 border border-surface-4 rounded-[14px] animate-pulse" />
              ))}
            </div>
          ) : campaigns.length === 0 ? (
            <div className="text-center py-16 text-text-muted">
              <Icon name="send" size={32} color="#4E4B58" />
              <p className="text-[14px] font-semibold mt-3">No campaigns yet</p>
              <p className="text-[12px] mt-1">Launch your first campaign above</p>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {campaigns.map((c) => (
                <div key={c.id} className="bg-surface-2 border border-surface-4 rounded-[14px] p-4 hover:border-[rgba(200,151,58,.2)] transition-all">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
                    <div>
                      <div className="font-extrabold text-[14px]">{c.name}</div>
                      <div className="text-[12px] text-text-muted mt-[2px]">{c.type} · {c.date}</div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[['Sent',c.sent,G.t1],['Opens',c.opens,G.bl],['Clicks',c.clicks,G.g]].map(([l,v,col]) => (
                      <div key={l} className="bg-surface-3 border border-surface-4 rounded-[9px] p-[11px] text-center">
                        <div className="font-display text-[18px] font-bold" style={{ color:col }}>{(v||0).toLocaleString()}</div>
      ...
      {tab === 'templates' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {BUILT_IN_TEMPLATES.map((t) => (
            <div key={t.name} className="bg-surface-2 border border-surface-4 rounded-[14px] p-[18px] flex flex-col hover:border-[rgba(200,151,58,.2)] transition-all">
              <div className="flex justify-between items-start mb-3">
                <div className="font-extrabold text-[13.5px]">{t.name}</div>
                <span className="px-[7px] py-[2px] rounded-[20px] text-[9.5px] font-extrabold uppercase"
                  style={{ background:`${t.color}18`, color:t.color }}>
                  {t.type}
                </span>
              </div>
              <div className="text-[12.5px] text-text-secondary leading-[1.55] flex-1 mb-4">{t.desc}</div>
              <Button variant="ghost" size="sm" className="justify-center w-full"
                onClick={() => { setInitForm({ type: t.type, name: t.name }); setWizOpen(true); }}>
                Use Template
              </Button>
            </div>
          ))}
        ...
        <div className="bg-surface-2 border border-surface-4 rounded-[14px] p-[22px]">
          ...
          {automations.length === 0 ? (
            <p className="text-[13px] text-text-muted text-center py-6">
              No automations configured. Go to the <strong>Automation Engine</strong> to create rules.
            </p>
          ) : (
            automations.map((a) => (
              <div key={a.id} className="flex items-center gap-3 py-[13px] border-b border-surface-4 last:border-0">
                <Toggle checked={a.enabled} onChange={() => toggleAutomation(a.id)} label={`Toggle: ${a.trigger}`} />
                <div className="flex-1">
                  <div className="font-bold text-[13.5px]">
                    {a.name || <>When: <span className="text-text-secondary font-normal">{a.trigger}</span></>}
                  </div>
                  {a.actions?.length > 0 && (
          ...
          <Button variant="gold" size="sm" className="mt-4"
            onClick={() => { setTab('campaigns'); window.location.href = '/automation'; }}>
            <Icon name="plus" size={12} />Manage in Automation Engine
          </Button>
        </div>
      )}

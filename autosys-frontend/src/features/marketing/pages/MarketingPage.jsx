...
import { CAMPAIGN_TYPES } from '@/shared/constants';
...
const STATUS_COLOR = { Active: G.ok, Completed: G.bl, Scheduled: G.wa, Draft: G.t2 };
...
function CampaignWizard({ open, onClose, initialForm = {} }) {
  ...
  const generateMsg = async () => {
    setAiLoad(true);
    try {
      const { data } = await aiApi.campaignMessage({
        name:     form.name,
        type:     form.type,
        audience: form.audience,
      });
      const text = data.text;
      setForm((f) => ({ ...f, msg: text }));
    } catch {
      setForm((f) => ({ ...f, msg: '🚗 Huge deals this January! Premium vehicles at unbeatable prices. Visit Dangote Motors — limited slots available!' }));
    }
    setAiLoad(false);
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title="New Campaign" maxWidth={560}>
        ...
        {WIZARD_STEPS.map((label, i) => (
          <div key={label} className="flex-1 flex flex-col items-center gap-1">
            <div
              ...
              style={{
                background: step >= i ? G.g : G.s4,
                color:      step >= i ? G.bg : G.t2,
              }}
            >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[['whatsapp','#25D366','wa','WhatsApp'],['email',G.bl,'mail','Email'],['instagram','#E1306C','img','Instagram'],['sms',G.wa,'send','SMS']].map(([type, color, icon, label]) => (
                <button
                  ...
                  className={cn(
                    'flex flex-col items-center gap-[5px] px-2 py-[11px] rounded-[9px]',
                    'border cursor-pointer transition-all font-sans',
                    form.type === type
                      ? 'text-[13px] font-bold'
                      : 'border-surface-4 bg-transparent text-text-secondary hover:bg-surface-3',
                  )}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-surface-3 rounded-[10px] p-4 border border-surface-4">
            ...
            <div className="font-display text-[28px] font-bold" style={{ color: G.g }}>
              {form.audience === 'all' ? 127 : form.audience === 'new_leads' ? 43 : 34}
            </div>
      ...
      <div className="flex justify-between mt-5">
        ...
        <Button variant="gold" onClick={() => step === WIZARD_STEPS.length - 1 ? handleLaunch() : setStep((s) => s + 1)}>
          {step === WIZARD_STEPS.length - 1 ? <><Icon name="send" size={14} />Launch Campaign</> : 'Continue →'}
        </Button>
      </div>
    </Modal>
  );
      ...
      {tab === 'campaigns' && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            {[['Sent',stats.sent.toLocaleString(),G.bl],['Opens',stats.opens.toLocaleString(),G.g],['Click Rate','22.4%',G.ok],['Unsubscribes',stats.unsubs,G.er]].map(([l,v,c]) => (
          ...
          <div className="flex flex-col gap-3">
            {campaigns.map((c) => (
              <div key={c.id} className="bg-surface-2 border border-surface-4 rounded-[14px] p-4 cursor-pointer hover:border-[rgba(200,151,58,.2)] transition-all">
          ...
          {TEMPLATES.map((t) => (
            <div key={t.name} className="bg-surface-2 border border-surface-4 rounded-[14px] p-[18px] flex flex-col hover:border-[rgba(200,151,58,.2)] transition-all">
              ...
              <Button
                ...
                onClick={() => { setInitForm({ type: t.type.toLowerCase(), name: t.name }); setWizOpen(true); }}
              >
                Use Template
              </Button>
            </div>
        ...
        <div className="bg-surface-2 border border-surface-4 rounded-[14px] p-[22px]">
          ...
          {automations.map((a) => (
            <div key={a.id} className="flex items-center gap-3 py-[13px] border-b border-surface-4 last:border-0">
              ...
              <Button variant="ghost" size="xs"><Icon name="edit" size={11} /></Button>
            </div>
          ))}
          <Button variant="gold" size="sm" className="mt-4" onClick={() => toast('Automation builder coming soon!')}>

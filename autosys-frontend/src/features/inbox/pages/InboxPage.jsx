...
/* ── Constants ─────────────────────────────────────────────── */
const CHANNELS = [
  { key:'all',      label:'All',      color:'#C8973A' },
  { key:'whatsapp', label:'WhatsApp', color:'#25D366' },
  { key:'sms',      label:'SMS',      color:'#3B82F6' },
  { key:'email',    label:'Email',    color:'#6366F1' },
];

const CH_COLOR = { whatsapp:'#25D366', sms:'#3B82F6', email:'#6366F1', call:'#F59E0B' };
const CH_ICON  = { whatsapp:'wa', sms:'phone', email:'report', call:'phone' };
...
const mapConv = (m) => {
  ...
  const time  = diff < 60000    ? 'Just now'
    ...
    <div className={cn('flex mb-3', isOut ? 'justify-end' : 'justify-start')}>
      {!isOut && (
        <div className="w-6 h-6 rounded-full bg-surface-3 border border-surface-4 flex items-center justify-center mr-2 shrink-0 mt-0.5">
          <Icon name={CH_ICON[msg.ch]||'phone'} size={11} color={CH_COLOR[msg.ch]} />
        </div>
      )}
      <div className={cn(
        ...
        <div className="flex items-center justify-end gap-1 mt-1">
          <p className={cn('text-[9.5px]', isOut ? 'text-gold/60' : 'text-text-muted')}>{msg.time}</p>
          ...
          {isOut && msg.status === 'sending'   && <span className="text-[9px] text-text-muted">…</span>}
...
function ConvItem({ conv, selected, onClick }) {
  const color = CH_COLOR[conv.channel];
  return (
    <button onClick={onClick}
      className={cn(
        'w-full text-left flex items-start gap-3 px-3 py-[10px] rounded-[10px] border transition-all',
        selected
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-[2px]">
          <Icon name={CH_ICON[conv.channel]||'phone'} size={10} color={color} />
        ...
        <span className="text-[9.5px] font-bold px-[5px] py-[1px] rounded-full mt-0.5 inline-block capitalize"
...
export function InboxPage() {
  ...
  useEffect(() => { if (selConv) fetchThread(selConv); }, [selConv, fetchThread]);
  ...
  const send = async () => {
    ...
    setSending(true);
    const optimistic = {
      id:`temp-${Date.now()}`, dir:'out', body:message,
      subject: sendCh === 'email' ? subject : null,
      time:'Now', ch:sendCh, status:'sending',
    };
    setThread((prev) => [...prev, optimistic]);
    const msg = message;
    setMessage('');
    try {
      await inboxApi.send({ leadId:selConv.lead_id, channel:sendCh, message:msg, subject: sendCh==='email' ? subject : undefined });
      setThread((prev) => prev.map((m) => m.id === optimistic.id ? { ...m, status:'sent' } : m));
      setConvs((prev) => prev.map((c) => c.lead_id === selConv.lead_id ? { ...c, last:msg } : c));
  ...
  const filtered    = convs.filter((c) => !search || c.name.toLowerCase().includes(search.toLowerCase()) || (c.last||'').toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="max-w-[1400px] px-4 md:px-[22px] pt-[22px] pb-[88px] md:pb-[22px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div>
          <h2 className="font-display text-[23px] font-bold">Unified Inbox</h2>
        ...
        <div className="flex gap-2 flex-wrap">
          {CHANNELS.map((ch) => (
            <button key={ch.key} onClick={() => { setChannel(ch.key); setSelConv(null); setThread([]); }}
              className={cn(
                'px-3 py-[6px] rounded-[8px] text-[11.5px] font-bold transition-colors border',
                channel === ch.key
      ...
      <div className="flex gap-3 h-[calc(100vh-180px)] min-h-[480px]">
        ...
        <div className="w-[280px] shrink-0 bg-surface-1 border border-surface-4 rounded-[14px] flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
            {convsLoading && Array(5).fill(0).map((_, i) => (
              <div key={i} className="flex gap-3 items-start p-2 animate-pulse">
                <div className="w-9 h-9 rounded-full bg-surface-3 shrink-0" />
                <div className="flex-1"><div className="h-3 bg-surface-3 rounded w-2/3 mb-1.5" /><div className="h-2.5 bg-surface-3 rounded w-full" /></div>
              </div>
            ))}
          ...
          {selConv ? (
            <div className="flex items-center gap-3 px-4 py-3 border-b border-surface-4 shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10.5px] text-text-muted truncate">{selConv.phone || selConv.email || '—'}</p>
              </div>
              {selConv.phone && (
                <button onClick={() => window.open(`tel:${selConv.phone}`)}
            ...
            <div className="px-4 py-3 border-b border-surface-4 text-[13px] text-text-muted">Select a conversation</div>
          )}

          {/* Messages */}
          <div className="flex-1 overflow-y-auto px-4 py-3">
            {threadLoading && <div className="flex justify-center py-8"><Spinner size={20} /></div>}
            {!threadLoading && !selConv && (
              <div className="flex flex-col items-center justify-center h-full text-center text-text-muted">
                <span className="text-[48px] mb-3">📭</span>
                <p className="text-[13px] font-bold">No conversation selected</p>
                <p className="text-[12px] mt-1">Messages from WhatsApp, SMS & Email appear here</p>
              </div>
            )}
            {!threadLoading && selConv && thread.length === 0 && (
          ...
          {selConv && (
            <div className="border-t border-surface-4 p-3 shrink-0">
              ...
              <div className="flex gap-1.5 mb-2">
                {['whatsapp','sms','email'].map((ch) => (
                  <button key={ch} onClick={() => setSendCh(ch)}
                    ...
                    <Icon name={CH_ICON[ch]} size={10} color={sendCh===ch ? CH_COLOR[ch] : '#4E4B58'} />
                    {ch}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={send} disabled={!message.trim() || sending} className="self-end">
                  {sending ? <Spinner size={13} /> : 'Send'}
                </Button>
              </div>
              {sendCh === 'whatsapp' && !selConv?.phone && <p className="text-[10px] text-yellow-400 mt-1">⚠️ No phone number for this lead</p>}
              {sendCh === 'email'    && !selConv?.email && <p className="text-[10px] text-yellow-400 mt-1">⚠️ No email address for this lead</p>}
            </div>
          )}
        </div>
      </div>
